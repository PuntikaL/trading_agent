...
def backtest(agent, data, stop_loss_ratio=0.02, take_profit_ratio=0.04):
    """Backtest a trading agent."""
    total_loss = 0
    risk_ratios = []
    portfolio_values = [agent.initial_cash]
    ...
    for _, row in data.iterrows():
        ...
        if signal == 1:  # Buy Signal
            entry_price = price
            risk_ratio = calculate_risk_ratio(entry_price, data)
        ...
        average_risk_ratio = np.mean(risk_ratios) if risk_ratios else 0

        # Calculate total loss if the portfolio value decreases
    ...
    return total_loss, max_loss, final_portfolio_value, average_risk_ratio
...
# Backtest each agent
total_loss_1m, max_loss_1m, portfolio_value_1m, risk_ratio_1m = backtest(agent_1m, df_1m)
total_loss_1m_DT, max_loss_1m_DT, portfolio_value_1m_DT, risk_ratio_1m_DT = backtest(agent_1m_DT, df_1m)
total_loss_1m_SARSA, max_loss_1m_SARSA, portfolio_value_1m_SARSA, risk_ratio_1m_SARSA = backtest(agent_1m_SARSA, df_1m)
total_loss_1h, max_loss_1h, portfolio_value_1h, risk_ratio_1h = backtest(agent_1h, df_1h)
total_loss_1d, max_loss_1d, portfolio_value_1d, risk_ratio_1d = backtest(agent_1d, df_1d)
total_loss_1h_RL, max_loss_1h_RL, portfolio_value_1h_RL, risk_ratio_1h_RL = backtest(agent_1h_RL, df_1h)
total_loss_1d_pca, max_loss_1d_pca, portfolio_value_1d_pca, risk_ratio_1d_pca = backtest(agent_1d_pca, df_1d)
total_loss_1d_RL, max_loss_1d_RL, portfolio_value_1d_RL, risk_ratio_1d_RL = backtest(agent_1d_RL, df_1d)
total_loss_1d_VPA, max_loss_1d_VPA, portfolio_value_1d_VPA, risk_ratio_1d_VPA = backtest(agent_1d_VPA, df_1d)
...
print(f"Risk Ratio for 1m Interval: {risk_ratio_1m:.2f}")
print(f"Risk Ratio for 1m Decision Tree Interval: {risk_ratio_1m_DT:.2f}")
print(f"Risk Ratio for 1m SARSA Interval: {risk_ratio_1m_SARSA:.2f}")
print(f"Risk Ratio for 1h Interval: {risk_ratio_1h:.2f}")
print(f"Risk Ratio for 1h RL Interval: {risk_ratio_1h_RL:.2f}")
print(f"Risk Ratio for 1d Interval: {risk_ratio_1d:.2f}")
print(f"Risk Ratio for 1d PCA Interval: {risk_ratio_1d_pca:.2f}")
print(f"Risk Ratio for 1d RL Interval: {risk_ratio_1d_RL:.2f}")
print(f"Risk Ratio for 1d VPA Interval: {risk_ratio_1d_VPA:.2f}")
